...
struct loopList_s//std::list 一个std::list栈帧呢包含唯一的ID，另外一个std::list包含栈帧中用到的变量实例
{
  UINT32 idLoop;
...
VOID WriteMem(UINT64 insAddr, std::string insDis, UINT32 opCount, REG reg_r, UINT64 memOp)
{
  ...
  UINT64 addr = memOp;
  PIN_LockClient();
  IMG img = IMG_FindByAddress(addr);//Find image by address. For each image, check if the address is within the mapped memory region of one of its segments.
  PIN_UnlockClient();
  SEC sec;
  //
  if (opCount != 2){
  ...
  for(sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec)){//IMG_SecHead()  First section in image
...
VOID PrologueAnalysis(UINT64 insAddr, UINT64 nextInsAddr, std::string insDis)// 序幕分析
{
  UINT64  i = 0;
...
VOID EpilogueAnalysis(UINT64 insAddr, UINT64 nextInsAddr, std::string insDis)//结语分析
{
  if (nextInsAddr >= 0x4004a0){
    #ifdef DEBUG
...
VOID ValueSetAnalysis(UINT64 insAddr, std::string insDis, ADDRINT rsp, ADDRINT rbp, UINT64 memOp)
{
  ...
  /* Ajout des variables de la stack frame dans une VSAL   VSAL中堆栈帧的jout变量*/
  /* stID est unique pour chaque stack frame  stID对于每个堆栈帧都是唯一的 */
  elem.addrVar = addrRBP;
  elem.sizeVar = 0;
  elem.rsp     = rsp;
  /* boucle pour eviter les doublons et eviter d'avoir plusieur 循环以避免重复，避免有多个*/
...
VOID Instruction(INS ins, VOID *v)//这一部分很关键，搞清楚具体是干嘛的，怎么调用的，都有哪些功能
{
  PIN_LockClient();
