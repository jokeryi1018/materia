...
VOID RecordMemRead(ADDRINT ip, VOID * addr)
{
	...
	if((prace[i].iflock == 1) && (prace[k].iflock == 1))
	    ...
	    iflock1 = 0;
	fprintf(trace,"(%d,%d)  (%ld,%ld)  %d  %d  (%d,%d)  %d  %d  (%d,%d)  %d  %d\n", i + 1, k + 1, prace[i].addre, prace[k].addre, ifop, ifmem, prace[i]._THREAD_id, prace[k]._THREAD_id, racecount, sum, prace[i].line, prace[k].line, iflock1, ifvc);
	}
    }
...
VOID RecordMemWrite(ADDRINT ip, VOID * addr)
{
   ...
   // prace[i]._disassemble = disassemble;
	...
	if((prace[i].iflock == 1) && (prace[k].iflock == 1))
	    ...
	    iflock1 = 0;
	fprintf(trace,"(%d,%d)  (%ld,%ld)  1  %d  (%d,%d)  %d  %d  (%d,%d)  %d  %d\n", i + 1, k + 1, prace[i].addre, prace[k].addre, ifmem, prace[i]._THREAD_id, prace[k]._THREAD_id, racecount, sum, prace[i].line, prace[k].line, iflock1, ifvc);
	}
    }
  /*  if((prace[i].line == prace[i - 1].line) && (prace[i - 1].memaddre == prace[i].memaddre) && (prace[i - 1].op == 'R'))
    {
