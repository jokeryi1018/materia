...
{
    int k = 0, j = 0, con = 0, con1 = 0, ifone = 0, ifvc = 0, ifmem = 0, *dra;
    //dra = new int[100];
    rep = 0;
    ifrace = 0;
	...
	if(prace[i].memaddre == prace[k].memaddre)
	    ...
	    ifmem = 0;
	fprintf(trace,"num-num:%d-%d  ip-ip:%ld-%ld  op-op:%c-%c  ifmem:%d  tid-tid:%d-%d  racecount:%d  sumtwain:%d  line-line:%d-%d  iflock:%d-%d  ifvc:%d  ifrace:%d\n", i + 1, k + 1, prace[i].addre, prace[k].addre, prace[i].op, prace[k].op, ifmem, prace[i]._THREAD_id, prace[k]._THREAD_id, racecount, sum, prace[i].line, prace[k].line, prace[i].iflock, prace[k].iflock, ifvc, ifrace);
	}
    }
