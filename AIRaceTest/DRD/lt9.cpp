...
FILE * trace;
PIN_LOCK pinLock;
int i = 0, Totalline = 0, sum=0, m = 0, lockvalue = 0, ct = 0, aa = 0, bb = 0, ifrace = 0;
...
{
    int k = 0, j = 0, con = 0, con1 = 0, ifone = 0, ifvc = 0, ifmem = 0, ifop = 0, iflock1 = 0, *dra;
    //dra = new int[100];
    rep = 0;
    ifrace = 0;
	...
	case 0:
		...
		if((tidc[ct] != tidc[ct - 1]) && ct && ((liflock[ct] == 1) && (liflock[ct - 1] == 1)))	   
		{
	        ...
	        if(vc[prace[i]._THREAD_id][ll] < vc[tidc[ct - 1]][ll])
	        vc[prace[i]._THREAD_id][ll] = vc[tidc[ct - 1]][ll];
	        else
	        vc[tidc[ct - 1]][ll] = vc[prace[i]._THREAD_id][ll];
	        }   
		ct++;
	        break;
		}
	...
	if(prace[i]._THREAD_id != prace[k]._THREAD_id)
	{
	    ...
	    if(((con1 == 0) || (prace[k].line != prace[k - 1].line)) && ((prace[i].line != prace[i - 1].line) || (prace[i - 1].memaddre != prace[i].memaddre)) && prace[k].line)
	    {
    ...
    if(ifone)
    {
	vc[prace[i]._THREAD_id][prace[i]._THREAD_id]++;
	ifone = 0;
	tidc[ct] = prace[i]._THREAD_id;
		liflock[ct] = prace[i].iflock;
		if((tidc[ct] != tidc[ct - 1]) && ct && ((liflock[ct] == 1) && (liflock[ct - 1] == 1)))	   
		{
	        for(int ll = 0; ll < 8; ll++)
	...
	/*    if(prace[k].memaddre == prace[i].memaddre && )
	   ...
	   // global = prace[i].memaddre;
		...
		if(((con == 0) || (prace[k].line != prace[k - 1].line)) && ((prace[i].line != prace[i - 1].line) || (prace[i - 1].memaddre != prace[i].memaddre)) && prace[k].line)
			...
			if(vc[prace[i]._THREAD_id][ll] < vc[prace[k]._THREAD_id][ll])
			    aa = 1;
			if(vc[prace[i]._THREAD_id][ll] > vc[prace[k]._THREAD_id][ll])
			    bb = 1;
		    }
		...
		if(aa == 1 && bb == 1)
		{
		  ...
		  //  report[m][0] = prace[k].line;
		   // report[m++][1] = prace[i].line;
		    ...
		    bb = 0;
		    ifvc = 1;
		  /*  fprintf(trace,"num-num:%d-%d  ip-ip:%ld-%ld  op-op:%c-%c  mem:%p  tid-tid:%d-%d  racecount:%d  sumtwain:%d  line-line:%d-%d  iflock:%d-%d  ifvc:%d\n", i + 1, k + 1, prace[i].addre, prace[k].addre, prace[i].op, prace[k].op, prace[i].memaddre, prace[i]._THREAD_id, prace[k]._THREAD_id, racecount, sum, prace[i].line, prace[k].line, prace[i].iflock, prace[k].iflock, ifvc);*/
		}
		else
		    ifvc = 0;
		}
		...
		con = 0;
	if(prace[i].memaddre == prace[k].memaddre)
	    ifmem = 1;
	else
	    ifmem = 0;
	if(prace[k].op == 'W')
	    ...
	    ifop = 0;
	if((prace[i].iflock == 1) && (prace[k].iflock == 1))
	    iflock1 = 1;
	else
	    iflock1 = 0;
	fprintf(trace,"(%d,%d)  (%ld,%ld)  %d  %d  (%d,%d)  %d  %d  (%d,%d)  %d  %d  %d\n", i + 1, k + 1, prace[i].addre, prace[k].addre, ifop, ifmem, prace[i]._THREAD_id, prace[k]._THREAD_id, racecount, sum, prace[i].line, prace[k].line, iflock1, ifvc, ifrace);
	}
    }

  /*  if(sum)
    {
        fprintf(trace,"num:%d  ip:%ld  op:%c  mem:%p  tid:%d  racecount:%d  sumtwain:%d  line:%d  iflock:%d  %d", i + 1, prace[i].addre, prace[i].op, prace[i].memaddre, prace[i]._THREAD_id, racecount, sum, prace[i].line, prace[i].iflock, ifvc);
...
{
    int k = 0, j = 0, con = 0, con1 = 0, ifone = 0, ifvc = 0, ifmem = 0, iflock1 = 0, *dra;
   // dra = new int[100];
    ifrace = 0;
    INT32* column = (INT32*)malloc(sizeof(INT32));
    std::string filename;
   ...
   // prace[i]._disassemble = disassemble;
	...
	if(prace[i]._THREAD_id != prace[k]._THREAD_id)
	{
	    ...
	    if(((con1 == 0) || (prace[k].line != prace[k-1].line)) && ((prace[i].line != prace[i - 1].line) || (prace[i - 1].memaddre != prace[i].memaddre) || (prace[i -1].op == 'R')) && prace[k].line)
	    {
	    vc[prace[k]._THREAD_id][prace[k]._THREAD_id]++;
	    ifone = 1;
	    tidc[ct] = prace[k]._THREAD_id;
	...
	if((prace[i].iflock == 1) && (prace[k].iflock == 1))
	    ...
	    iflock1 = 0;
	fprintf(trace,"(%d,%d)  (%ld,%ld)  1  %d  (%d,%d)  %d  %d  (%d,%d)  %d  %d  %d\n", i + 1, k + 1, prace[i].addre, prace[k].addre, ifmem, prace[i]._THREAD_id, prace[k]._THREAD_id, racecount, sum, prace[i].line, prace[k].line, iflock1, ifvc, ifrace);
	}
    }
  /*  if((prace[i].line == prace[i - 1].line) && (prace[i - 1].memaddre == prace[i].memaddre) && (prace[i - 1].op == 'R'))
    {
