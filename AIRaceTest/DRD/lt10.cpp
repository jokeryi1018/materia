...
VOID RecordMemRead(char * ss, ADDRINT ip, VOID * addr)
{
    int k = 0, j = 0, con = 0, con1 = 0, ifone = 0, *dra;
    //dra = new int[100];
    rep = 0;
    ...
    //prace[i]._disassemble = ss;
    strcpy(prace[i]._disassemble, ss);
    ...
    if(sum)
    {
        fprintf(trace,"num:%d  ip:%ld  op:%c  mem:%p  tid:%d  disa:%s  racecount:%d  sumtwain:%d  line:%d  iflock:%d", i + 1, prace[i].addre, prace[i].op, prace[i].memaddre, prace[i]._THREAD_id, prace[i]._disassemble, racecount, sum, prace[i].line, prace[i].iflock);
	...
	fprintf(trace,"num:%d  ip:%ld  op:%c  mem:%p  tid:%d  disa:%s  racecount:%d  sumtwain:%d  line:%d  iflock:%d\n", i + 1, prace[i].addre, prace[i].op, prace[i].memaddre, prace[i]._THREAD_id, prace[i]._disassemble, racecount, sum, prace[i].line, prace[i].iflock);
...
VOID RecordMemWrite(char * ss, ADDRINT ip, VOID * addr)
{
    int k = 0, j = 0, con = 0, con1 = 0, ifone = 0, *dra;
   // dra = new int[100];
...
VOID ImageLoad(IMG img, VOID *)
{
	...
	for (SEC sec = IMG_SecHead(img); SEC_Valid(sec); sec = SEC_Next(sec))
        ...
        // executed before the IPOINT_BEFORE.
		  ...
		  //  UINT32 memOp;  
		  PIN_LockClient();		  
		  PIN_GetSourceLocation(INS_Address(ins), NULL, &prace[i].line, NULL);
		  PIN_UnlockClient();
		  if(prace[i].line)        	  
		  {
		  // string s = (INS_Disassemble(ins)).c_str();
		 // cout<<s<<endl;
		  if( INS_IsMemoryRead(ins) ) 
			INS_InsertCall( ins, IPOINT_BEFORE, (AFUNPTR)RecordMemRead,
                                   IARG_ADDRINT, a, IARG_INST_PTR, IARG_MEMORYREAD_EA, IARG_END);
            	  if( INS_IsMemoryWrite(ins) ) 
			INS_InsertCall( ins, IPOINT_BEFORE, (AFUNPTR)RecordMemWrite,
                                  IARG_ADDRINT, a, IARG_INST_PTR, IARG_MEMORYWRITE_EA, IARG_END);
		  //cout<<s<<endl;
		  }
		//  delete s;
                }
